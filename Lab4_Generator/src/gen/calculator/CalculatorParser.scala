...
case class CalculatorParser[F[_]: Monad](inputStream: InputStream) {
  ...
  def log(in : Double) : CalculatorParseState[LogContext] = {
    for { 
      lex <- StateT.get[CalculatorMET, CalculatorLexer]
      res <- 
        if (lex.curTokenIn(Set(CalculatorToken.LOG))) {
          for {
            ...
            _ <-
              if (!lexer.compareToken(CalculatorToken.LOG)) {
                throwPEStateT[F, CalculatorLexer, List[GrammarTree[_]]]("Expected LOG, found:" + lexer.curToken().text)
              } else {
                StateT.pure[CalculatorMET, CalculatorLexer, List[GrammarTree[_]]](curState)
              }
            curState <- StateT.pure[CalculatorMET, CalculatorLexer, List[GrammarTree[_]]](curState)
          ...
          throwPEStateT[F, CalculatorLexer, LogContext]("Unexpected token: " + lex.curToken())
        }
    } yield res
  }
...
object CalculatorParser {
  ...
  case class LogContext(ctxRoot: String, ctxChildren: List[GrammarTree[_]] = List.empty, res: Double = 0)
  extends ContextTree(ctxRoot, ctxChildren) {
