...
case class CalculatorParser(inputStream: InputStream) {
	...
	def exp(in : Double) : ExpContext = {
		var ctx = ExpContext("exp")
		if(lex.curTokenIn(Set(CalculatorToken.POW))) {
			if(!lex.compareToken(CalculatorToken.POW)) {
				throw new ParseException("Expected POW, found:" + lex.curToken().text, lex.curPos())
			}
			ctx = ctx.appendLastChild(TerminalTree(lex.curToken()))
			val p = lex.curToken()
			lex.nextToken()
...
object CalculatorParser {
	...
	case class ExpContext(ctxRoot: String, ctxChildren: List[GrammarTree[_]] = List.empty) extends ContextTree(ctxRoot, ctxChildren) {
		var res: Double = 0
		override def pushFirstChild(child: GrammarTree[_]) = ExpContext(ctxRoot, child::ctxChildren)
		override def appendLastChild(child: GrammarTree[_]) = ExpContext(ctxRoot, ctxChildren ++ List(child))
	 }
